...
# 
# Main logic:
# - processes all tags from the original opus repo
# - checks out each tag
...
# Get all tags in original/opus
def get_opus_tags():
    result = subprocess.run(['git', 'tag'], cwd=OPUS_DIR, stdout=subprocess.PIPE, text=True, check=True)
